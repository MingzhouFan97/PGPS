...
class IIDGaussian():
    ...
    def log_prob(self, X):
        return -torch.sum((X)**2/self.scale, dim=1)
    def sample(self, num):
        return torch.sqrt(torch.tensor(self.scale)) * torch.randn([num[0], self.dim])
...
class Inference():
    def __init__(self, base_model, model_paras, train_x, train_y, num, device=None, classification=True, self_p0=False):
        model_illu = base_model(*model_paras)
        vec = get_par_vec(model_illu)
        self.dim = vec.shape[0]
        if self_p0:
            self.P0 = IIDGaussian(self.dim, init_var)
        else:
