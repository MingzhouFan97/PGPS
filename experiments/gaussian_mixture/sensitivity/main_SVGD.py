...
if __name__ == "__main__":
    ...
    def adaptive_step_path(path, name):
        ...
        for j in range(7000):
            ...
            lrr.append(torch.sum(samples<0)/samples_num)
        samples = pggf.X.detach().squeeze()
        probs = KDE(samples, x)

        torch.save(probs, './experiment/weight_reserving_toy/probs_SVGD.ts')
        torch.save(torch.tensor(lrr), './experiment/weight_reserving_toy/lrr_SVGD.ts')
