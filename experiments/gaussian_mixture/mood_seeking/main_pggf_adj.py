...
if __name__ == "__main__":
    ...
    def adaptive_step_path(size, path, name, adjust_steps=0):
        ...
        if device == 'cpu':
            ...
            torch.save(samples.clone().detach(), f'./experiment/mode_seeking_toy/samples_{name}.ts')
            torch.save(probs.clone().detach(), f'./experiment/mode_seeking_toy/probs_{name}.ts')
            torch.save(iteration, f'./experiment/mode_seeking_toy/iteration_{name}.ts')
    ...
    if device == 'cuda':
        path = pggfp.ExpPath
        adaptive_step_path(torch.tensor(1.), path, 'exp_1_adj', adjust_steps=adj_steps)
        adaptive_step_path(torch.tensor(.5), path, 'exp_p5_adj', adjust_steps=adj_steps)
        adaptive_step_path(torch.tensor(.1), path, 'exp_p1_adj', adjust_steps=adj_steps)
        adaptive_step_path(torch.tensor(.05), path, 'exp_p05_adj', adjust_steps=adj_steps)
        adaptive_step_path(torch.tensor(.01), path, 'exp_p01_adj', adjust_steps=adj_steps)
        #
        path = pggfp.ExpTelePath
        adaptive_step_path(torch.tensor(1.), path, 'tel_1_adj', adjust_steps=adj_steps)
        adaptive_step_path(torch.tensor(.5), path, 'tel_p5_adj', adjust_steps=adj_steps)
        adaptive_step_path(torch.tensor(.1), path, 'tel_p1_adj', adjust_steps=adj_steps)
        adaptive_step_path(torch.tensor(.05), path, 'tel_p05_adj', adjust_steps=adj_steps)
        adaptive_step_path(torch.tensor(.01), path, 'tel_p01_adj', adjust_steps=adj_steps)
    else:
        path = pggfp.ExpTelePath
        adaptive_step_path(torch.tensor(.1), path, 'tel_p1_adj', adjust_steps=adj_steps)
        adaptive_step_path(torch.tensor(.05), path, 'tel_p05_adj', adjust_steps=adj_steps)
